...
import { Upload, Download, FileSpreadsheet, Users, Filter, Search, Eye, Edit, Trash2, Mail, TrendingUp } from 'lucide-react';
      ...
      {stats && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{stats.newLeads.toLocaleString()}</div>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <CardContent>
              <div className="text-2xl font-bold">{stats.contactedLeads.toLocaleString()}</div>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <CardContent>
              <div className="text-2xl font-bold">{stats.convertedLeads.toLocaleString()}</div>
            </CardContent>
          </Card>
        </div>
      )}
        ...
        <TabsContent value="upload" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Upload your leads from an Excel or CSV file. Supported columns: Name, Email, Phone, Company, Country, State, City, Industry, Job Title, Lead Source.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              >
                ...
                {isDragActive ? (
                  ...
                  <div>
                    <p className="text-lg mb-2">Drag & drop your leads file here, or click to browse</p>
                    <p className="text-sm text-gray-500">Supports .xlsx, .xls, and .csv files (max 10MB)</p>
                  </div>
                )}
              </div>
                  ...
                  <AlertDescription>
                    <div className="space-y-2">
                      ...
                      {uploadStatus.status === 'PROCESSING' && (
                        <Progress value={(uploadStatus.processedLeads / Math.max(uploadStatus.totalLeads, 1)) * 100} />
                      )}
                      {uploadStatus.status === 'COMPLETED' && (
                        <p className="text-sm">
                          Successfully processed {uploadStatus.processedLeads} leads.
                          {uploadStatus.failedLeads > 0 && ` ${uploadStatus.failedLeads} failed.`}
                        </p>
                      )}
                    </div>
                  </AlertDescription>
                ...
                <Table>
                  ...
                  <TableBody>
                    {loading ? (
                      ...
                      <TableRow>
                        <TableCell colSpan={8} className="text-center py-8">
                          No leads found. Try uploading some leads or adjusting your filters.
                        </TableCell>
                      </TableRow>
                    ) : (
                      leads.map((lead) => (
                        <TableRow key={lead.id}>
                          ...
                          <TableCell>
                            {lead.emailsSent?.[0]?.createdAt
                              ? new Date(lead.emailsSent[0].createdAt).toLocaleDateString()
                              : 'Never'
                            }
                          </TableCell>
                          <TableCell>
