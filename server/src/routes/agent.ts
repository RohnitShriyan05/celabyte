...
import { tenantRateLimit, validateQuery, sanitizeInput, requestTimeout } from "../middleware/security"
...
router.post("/", 
  ...
  try {
    ...
    if (provider === "gemini" && env.geminiKey) {
      result = await handleGeminiQuery(req.user.tenantId, message, req.user.id, selectedConnections)
