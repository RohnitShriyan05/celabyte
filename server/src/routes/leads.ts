...
// Get upload status
router.get('/upload/:uploadId/status', authenticateToken, async (req: Request, res: Response) => {
  try {
...
async function processLeadsFile(filePath: string, tenantId: string, uploadId: string, mimeType: string) {
  try {
    let leads: any[] = [];
    ...
    if (mimeType.includes('sheet') || mimeType.includes('excel')) {
      ...
      worksheet.eachRow((row, rowNumber) => {
        ...
        row.eachCell((cell, colNumber) => {
          ...
          else if (header.includes('company')) lead.company = String(value || '');
          else if (header.includes('country')) lead.country = String(value || '');
          else if (header.includes('state')) lead.state = String(value || '');
          else if (header.includes('city')) lead.city = String(value || '');
          else if (header.includes('industry')) lead.industry = String(value || '');
          else if (header.includes('job') || header.includes('title')) lead.jobTitle = String(value || '');
          else if (header.includes('source')) lead.leadSource = String(value || '');
        });
