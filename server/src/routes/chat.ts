...
import { tenantRateLimit, sanitizeInput, validateQuery, requestTimeout } from "../middleware/security";
...
router.post("/", requireRole("VIEWER"), async (req, res) => {
  try {
    ...
    logger.error({ error: error.message, tenantId: req.user?.tenantId }, "Failed to create chat");
...
router.get("/", requireRole("VIEWER"), async (req, res) => {
  try {
    ...
    logger.error({ error: error.message, tenantId: req.user?.tenantId }, "Failed to get conversations");
    res.status(400).json({ error: error.message });
  }
});

// Get messages for a specific conversation
router.get("/:conversationId/messages", requireRole("VIEWER"), async (req, res) => {
  try {
    ...
    logger.error({ error: error.message, tenantId: req.user?.tenantId }, "Failed to get messages");
    res.status(400).json({ error: error.message });
  }
});
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (provider === "gemini" && env.geminiKey) {
        result = await handleGeminiQuery(req.user.tenantId, message, req.user.id, undefined, conversationId);
      } else if (provider === "openai" && env.openaiKey) {
        result = await handleAgentQuery(req.user.tenantId, message, req.user.id);
      ...
      await auditLog({
        ...
        params: { message: message.substring(0, 100), provider, conversationId },
...
router.patch("/:conversationId", requireRole("VIEWER"), async (req, res) => {
  try {
    ...
    logger.error({ error: error.message, tenantId: req.user?.tenantId }, "Failed to update conversation");
    res.status(400).json({ error: error.message });
  }
});
...
router.delete("/:conversationId", requireRole("VIEWER"), async (req, res) => {
  try {
    ...
    logger.error({ error: error.message, tenantId: req.user?.tenantId }, "Failed to delete conversation");
    res.status(400).json({ error: error.message });
  }
});
