...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const [totalEmails, sentEmails, failedEmails, templateStats, dailyStats] =
        await Promise.all([
      ...
      const templateIds = templateStats
        .map((stat) => stat.templateId)
        .filter(Boolean);
      ...
      const templateStatsWithNames = templateStats.map((stat) => ({
        ...stat,
        templateName:
          templates.find((t) => t.id === stat.templateId)?.name || "Unknown",
      }));
